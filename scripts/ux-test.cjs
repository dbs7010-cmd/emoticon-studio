...
(async()=>{
 ...
 await press('대사 추천');const dialogue=tree.root.findAll(n=>n.type==='TextInput'&&n.props.accessibilityLabel==='대사')[0].props.value;assert.equal(dialogue,'고마워!');
 pendingPicks=[{...image,path:'r2.png'},{...image,path:'r3.png'}];pendingPicks.forEach(i=>files.set(localFile(i.path).uri,png));
 await press('레퍼런스');await press('+ 참고 추가');
 ...
 await press('Reference 1 숨기기');assert.equal(lastCommand('transform').visible,false);
 scene.references[0].visible=false;
 await press('선택 Reference 2');scene.activeLayerId=scene.references[1].id;await message(scene);
 await press('활성 레이어 이동');assert.equal(lastCommand('tool').tool,'move');
 await press('레이어');await press('선택 Draw Layer 2');scene.activeLayerId='draw-2';await message(scene);await press('패널 닫기');
 await press('펜 종류');assert.equal(lastCommand('brush').brush,'marker');await press('지우개');
 await act(async()=>tree.root.findByType('Slider').props.onValueChange(30));assert.equal(lastCommand('width').tool,'eraser');
 await press('펜');assert.equal(tree.root.findByType('Slider').props.value,5,'pen/eraser widths independent');
 ...
 await press('다음 슬롯');await saved();const next=await init();assert.equal(next.layers[0].data,undefined);
 await press('이전 슬롯');await saved(false);const reopened=await init();assert.equal(reopened.references[0].visible,false);assert.equal(reopened.activeLayerId,'draw-2');
 ...
 console.log('PASS: 32 direct Painter entries, dialogue recommendation, multiple reference registration/control, independent widths, layered file persistence, export thumbnail, save-stay-next-previous-home, restart, old data compatibility. Native modules/WebView mocked; engine pixels tested separately.');
